...
import type { ChartType } from "@/components/custom-report/report-config";
...
export default function EmbedPage() {
  ...
  const canRun = selectedDimensions.length > 0 || selectedMetrics.length > 0;
  ...
  return (
    <div className="min-h-screen bg-page text-text-primary p-4">
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        <h1 className="text-lg font-semibold">DataPulse Report</h1>
      ...
      <div className="space-y-4">
        ...
        >
          ...
          {queryLoading ? "Running..." : "Generate Report"}
        </button>
        ...
        {!result && !error && !queryLoading && (
          <div className="flex items-center justify-center h-64 text-text-secondary text-sm">
            Select measures and groupings, then generate a report
          </div>
        )}
      </div>
    </div>
