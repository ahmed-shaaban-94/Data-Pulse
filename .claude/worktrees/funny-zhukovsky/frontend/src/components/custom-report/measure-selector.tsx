...
export function MeasureSelector({
  ...
  // Collect ungrouped metrics (exist in API but not in any predefined group)
  const ungrouped = availableMetrics
    .filter((m) => isUngroupedMetric(m.name))
    .map((m) => m.name);
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        ...
        {/* Other — catch any API metrics not in predefined groups */}
        {ungrouped.length > 0 && (
          <div>
            <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-widest text-text-secondary/60">
              Other
