...
def _reset_cache_state():
    ...
    Also patch get_settings to return empty redis_url so tests don't
    accidentally connect to a real Redis instance in CI.
    """
    cache_mod._redis_client = None
    cache_mod._last_attempt = 0
    with patch("datapulse.cache.get_settings") as mock_s:
        mock_s.return_value = MagicMock(redis_url="", redis_default_ttl=300)
        yield
    cache_mod._redis_client = None
    cache_mod._last_attempt = 0
