...
/** /control-center/sync-runs — Per-connection sync history on the v2 shell. */
...
function SyncHistory({ connectionId }: { connectionId: number }) {
  ...
  if (!data?.items.length)
    return <p className="px-4 py-3 text-xs text-text-secondary italic">No sync runs yet.</p>;
        ...
        {data.items.map((job: SyncJob) => (
          <tr key={job.id} className="border-b border-border/20">
            <td className="px-4 py-2 font-mono text-text-secondary">
              {job.pipeline_run_id?.slice(0, 8) ?? "—"}
            </td>
            <td className="px-4 py-2 text-text-secondary">{job.run_mode}</td>
            <td className="px-4 py-2">
              <span
                className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[job.status ?? ""] ?? "text-text-secondary"}`}
              >
                {job.status ?? "pending"}
              </span>
            </td>
...
export default function SyncRunsPage() {
  ...
  return (
    <DashboardShell
      activeHref="/control-center/sync-runs"
      breadcrumbs={[
    ...
    >
      <div className="page">
        <div>
          ...
          <p className="page-sub">Per-connection pipeline execution history, auto-refreshed every 15s.</p>
        </div>
          ...
          <div className="space-y-3">
            ...
            {data.items.length === 0 && (
              <p className="py-12 text-center text-text-secondary">
                No active connections. Add sources first.
              </p>
            )}
          </div>
        )}
