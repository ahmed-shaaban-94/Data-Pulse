...
function configPath(drugCode: string, siteCode: string) {
  return `${PATH}/${drugCode}/${siteCode}`;
}
...
async function fetchReorderConfig(filters?: ReorderConfigFilters): Promise<ReorderConfig | null> {
  ...
  try {
    return await fetchAPI<ReorderConfig>(configPath(filters.drug_code, filters.site_code));
  } catch (error) {
...
export function useReorderConfig(filters?: ReorderConfigFilters) {
  ...
  const key = enabled && filters?.drug_code && filters?.site_code
    ? swrKey(configPath(filters.drug_code, filters.site_code))
    : null;
