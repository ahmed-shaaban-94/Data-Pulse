...
describe("QuickPickGrid", () => {
  it("renders 8 tiles with numeric shortcut badges", () => {
    ...
    expect(screen.getByText("Drug 8")).toBeInTheDocument();
  });
  ...
  it("pads the grid with placeholder slots when catalog has fewer than 8 items", () => {
    ...
    // Slots 4-8 exist as placeholders but are not interactive
  ...
  it("includes the catalog price in each tile's aria-label for screen readers", () => {
    render(<QuickPickGrid items={CATALOG} onPick={vi.fn()} />);
    expect(screen.getByLabelText(/Quick pick 1.*Drug 1/)).toBeInTheDocument();
  });

  it("does not crash when the catalog is empty", () => {
    render(<QuickPickGrid items={[]} onPick={vi.fn()} />);
    // No real tiles, only placeholder slots — the grid renders its header
