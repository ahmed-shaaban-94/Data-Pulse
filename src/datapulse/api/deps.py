...
def get_sample_load_service(
    ...
    """Wires SampleLoadService together with a pipeline service + quality repo.

    SampleLoadService needs the raw session (for direct inserts of the
    sample rows), a PipelineService (to create the pipeline_run that
    tracks the load), and the QualityRepository (to seed the synthetic
    quality_checks row). Keeping this in deps.py avoids the route file
    pulling in three repositories.
    """
...
def get_notification_service(
    ...
    return NotificationService.for_session(session)
...
def _get_public_branding_session() -> Generator[Session, None, None]:
    """Unauthenticated DB session for public branding lookups (no RLS).

    Kept separate from ``get_plain_session`` because the public-branding
    path runs with a tighter 10s timeout (login page; fast fail preferred).
    """
    session = get_session_factory()()
    try:
...
def get_public_branding_service(
    session: Annotated[Session, Depends(_get_public_branding_session)],
):
    from datapulse.branding.repository import BrandingRepository
    from datapulse.branding.service import BrandingService
