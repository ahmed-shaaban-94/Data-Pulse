...
class PosRepository:
    ...
    # ──────────────────────────────────────────────────────────────
    # Pharmacist PIN lookup + upgrade (B7)
    # ──────────────────────────────────────────────────────────────

    def get_pharmacist_pin_hash(self, pharmacist_id: str) -> PinRecord | None:
        """Return a :class:`PinRecord` for a pharmacist member, or None.

        Looks up ``tenant_members.pharmacist_pin_hash``,
        ``tenant_members.pharmacist_pin_salt``, and
        ``tenant_members.pharmacist_pin_hash_algo`` by ``user_id``.
    ...
    def upgrade_pharmacist_pin(
        self, pharmacist_id: str, new_hash: str, new_salt: str
    ) -> None:
