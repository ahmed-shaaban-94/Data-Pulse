...
class Settings(BaseSettings):
    ...
    # Legacy provider flag kept only so stale AUTH_PROVIDER=clerk env files
    # fail loudly at startup. Auth0 is the only supported runtime provider.
    ...
    # Legacy Clerk runtime vars are retained only so startup validation can
    # reject stale dual-provider env files after issue #654.
    ...
    @model_validator(mode="after")
    def _reject_legacy_auth_provider_config(self) -> "Settings":
        """Fail fast when stale Clerk-era runtime config sneaks back in."""
        legacy_clerk_vars = {
            "CLERK_PUBLISHABLE_KEY": self.clerk_publishable_key,
            "CLERK_SECRET_KEY": self.clerk_secret_key,
            "CLERK_FRONTEND_API": self.clerk_frontend_api,
            "CLERK_JWT_ISSUER": self.clerk_jwt_issuer,
            "CLERK_JWT_AUDIENCE": self.clerk_jwt_audience,
            "CLERK_JWT_TEMPLATE": self.clerk_jwt_template,
        }
        configured_legacy = sorted(name for name, value in legacy_clerk_vars.items() if value)

        if self.auth_provider != "auth0":
            raise ValueError(
                "AUTH_PROVIDER must be 'auth0'. Clerk runtime support was removed; "
                "clear AUTH_PROVIDER or set it back to auth0."
            )
        if configured_legacy:
            raise ValueError(
                "Legacy Clerk runtime variables are no longer supported: "
                f"{', '.join(configured_legacy)}. Remove them from .env and use Auth0."
            )
        return self
    ...
    def _jwt_provider_configured(self) -> bool:
        """Whether Auth0 has enough config to verify Bearer tokens."""
